...
namespace Infrastructure.Repositories {
  public class UserRepository : IUserRepository {
    ...
    public async Task<bool> CheckUserOwnsArticle(int articleId, string userId) {
      var article =
          await _dbContext.Articles.AsNoTracking().SingleOrDefaultAsync(
              a => a.Id == articleId);
    ...
    public async Task<AuthResult> CreateAsync(string email, string password) {
      ...
      if (existingUser != null) {
        return new AuthResult{Errors =
                                  new[]{"User with this email address exists"}};
      }
    ...
    public async Task<AuthResult> LoginAsync(string email, string password) {
      ...
      if (user == null) {
        return new AuthResult{Errors = new[]{"User does not exist"}};
      }
    ...
    public async Task<AuthResult> RefreshTokenAsync(string token,
      ...
      var jti = validatedToken.Claims
                    .Single(t => t.Type == JwtRegisteredClaimNames.Jti)
                    .Value;

      var storedRefreshToken =
          await _dbContext.RefreshTokens.SingleOrDefaultAsync(
      ...
      var user = await _userManager.FindByIdAsync(
          validatedToken.Claims.Single(t => t.Type == "id").Value);
      return await GenerateAuthResultAsync(user);
    }
