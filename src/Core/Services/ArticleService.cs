...
namespace Core.Services {
  public class ArticleService : IArticleService {
    ...
    public async Task<ArticleResource>
    AddArticle(string userId, AddArticleResource addArticleResource,
    ...
    public async Task<ArticleResource>
    DeleteArticle(int id, CancellationToken cancellationToken = default) {
      var article = await _articleRepository.RemoveAsync(id, cancellationToken);
      return _mapper.Map<ArticleResource>(article);
    }

    public async Task<ArticleResource>
    GetArticle(int id, CancellationToken cancellationToken = default) {
      var article =
          await _articleRepository.GetByIdAsync(id, cancellationToken);
      return _mapper.Map<ArticleResource>(article);
    }

    public async Task<ArticleResource>
    UpdateArticle(int id, AddArticleResource articleResource,
